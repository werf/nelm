...
func BuildLegacyReleaseFromRelease(rel *Release) (*legacyRelease.Release, error) {
	legacyHooks := make([]*legacyRelease.Hook, 0, len(rel.hookCRDs)+len(rel.HookManifests()))
	for _, hook := range rel.HookCRDs() {
		legacyHook, err := crdHookToLegacyHook(hook, rel)
		if err != nil {
			return nil, fmt.Errorf("error converting hook to legacy hook: %w", err)
		}
		legacyHooks = append(legacyHooks, legacyHook)
	}
	for _, hook := range rel.HookResources() {
		legacyHook, err := resourceHookToLegacyHook(hook, rel)
		if err != nil {
			return nil, fmt.Errorf("error converting hook to legacy hook: %w", err)
		}
		legacyHooks = append(legacyHooks, legacyHook)
...
// FIXME(ilya-lesikov): to interface
func crdHookToLegacyHook(hook *resourcev2.LocalHookCRD, rel *Release) (*legacyRelease.Hook, error) {
...
func resourceHookToLegacyHook(hook *resourcev2.LocalHookResource, rel *Release) (*legacyRelease.Hook, error) {
	var deletePolicies []legacyRelease.HookDeletePolicy
	if hook.ShouldRecreate() {
		deletePolicies = append(deletePolicies, legacyRelease.HookBeforeHookCreation)
	}
