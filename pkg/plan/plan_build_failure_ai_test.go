...
func TestAI_BuildFailurePlan(t *testing.T) {
	failedRelease := &release.VersionedRelease{
		Accessor: lo.Must(helmrel.NewAccessor(&helmrelease.Release{
			Name:      "test-release",
			Namespace: "test-namespace",
			Version:   1,
			Info:      &helmrelease.Info{Status: helmreleasestatus.StatusPendingInstall},
		})),
	}
	failedReleaseID := releaseID("test-namespace", "test-release", 1)
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			expectedOperations: []string{
				OperationID(OperationTypeUpdateRelease, OperationVersionUpdateRelease, 0, failedReleaseID),
			},
		},
		{
...
func createReleaseTestOp(rel *release.VersionedRelease, status OperationStatus) *Operation {
	return &Operation{
...
func updateReleaseTestOp(rel *release.VersionedRelease, status OperationStatus) *Operation {
	return &Operation{
