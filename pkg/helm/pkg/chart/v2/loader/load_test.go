...
func TestLoadFiles_BadCases(t *testing.T) {
	for _, tt := range []struct {
		...
		_, err := LoadFiles(context.Background(), tt.bufferedFiles)
		if err == nil {
...
`),
	...
	c, err := LoadFiles(context.Background(), goodFiles)
	if err != nil {
	...
	if _, err = LoadFiles(context.Background(), []*archive.BufferedFile{}); err == nil {
...
`),
	...
	_, err = LoadFiles(context.Background(), goodFiles)
	if err != nil {
		t.Errorf("Expected good files to be loaded, got %v", err)
	}
	w.Close()
...
func TestLoadFileBackslash(t *testing.T) {
	c, err := Load(context.Background(), "testdata/frobnitz_backslash-1.2.3.tgz")
	if err != nil {
		t.Fatalf("Failed to load testdata: %s", err)
	}
...
func TestLoadInvalidArchive(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		_, err := Load(context.Background(), illegalChart)
		if err == nil {
