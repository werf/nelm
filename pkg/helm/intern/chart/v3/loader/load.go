...
func LoadFiles(ctx context.Context, files []*archive.BufferedFile) (*chart.Chart, error) {
	...
	for _, f := range files {
		...
		case strings.HasPrefix(f.Name, "templates/"):
			c.Templates = append(c.Templates, &chartcommon.File{Name: f.Name, Data: f.Data, ModTime: f.ModTime})
		case strings.HasPrefix(f.Name, "charts/"):
			if filepath.Ext(f.Name) == ".prov" {
				c.Files = append(c.Files, &chartcommon.File{Name: f.Name, Data: f.Data, ModTime: f.ModTime})
				continue
			}
	...
	if applyWerfExtensions {
		...
		case nelmcommon.LegacyChartTypeChartStub:
			...
			c.Metadata = autosetChartMetadata(
				c.Metadata,
				autosetChartMetadataOptions{
					DefaultAPIVersion: chart.APIVersionV3,
