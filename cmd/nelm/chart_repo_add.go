...
func newChartRepoAddCommand(ctx context.Context, afterAllCommandsBuiltFuncs map[*cobra.Command]func(cmd *cobra.Command) error) *cobra.Command {
	...
	cmd := lo.Must(lo.Find(repoCmd.Commands(), func(c *cobra.Command) bool {
		return strings.HasPrefix(c.Use, "add")
	}))
