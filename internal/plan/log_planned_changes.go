...
func LogPlannedChanges(
	...
	for _, change := range createdChanges {
		log.Default.InfoBlock(ctx, log.BlockOptions{
			BlockTitle: createStyle("Create ") + resourceStyle(change.IDHuman()) + ending(change.CleanedUpOnSuccess, change.CleanedUpOnFailure),
		}, func() {
	...
	for _, change := range recreatedChanges {
		log.Default.InfoBlock(ctx, log.BlockOptions{
			BlockTitle: recreateStyle("Recreate ") + resourceStyle(change.IDHuman()) + ending(change.CleanedUpOnSuccess, change.CleanedUpOnFailure),
		}, func() {
			log.Default.Info(ctx, "%s", change.Udiff)
		})
	}
	...
	for _, change := range updatedChanges {
		log.Default.InfoBlock(ctx, log.BlockOptions{
			BlockTitle: updateStyle("Update ") + resourceStyle(change.IDHuman()) + ending(change.CleanedUpOnSuccess, change.CleanedUpOnFailure),
		}, func() {
			log.Default.Info(ctx, "%s", change.Udiff)
		})
	}
	...
	for _, change := range appliedChanges {
		log.Default.InfoBlock(ctx, log.BlockOptions{
			BlockTitle: applyStyle("Blindly apply ") + resourceStyle(change.IDHuman()) + ending(change.CleanedUpOnSuccess, change.CleanedUpOnFailure),
		}, func() {
			log.Default.Info(ctx, "%s", change.Udiff)
		})
	}
	...
	for _, change := range deletedChanges {
		log.Default.InfoBlock(ctx, log.BlockOptions{
			BlockTitle: deleteStyle("Delete ") + resourceStyle(change.IDHuman()),
		}, func() {
			log.Default.Info(ctx, "%s", change.Udiff)
		})
	}
