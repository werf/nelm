...
func executeInGoja(vendorBundle, appBundle string, renderCtx map[string]interface{}) (interface{}, error) {
	...
	if vendorBundle != "" {
		if _, err := vm.RunString(vendorBundle); err != nil {
			return nil, fmt.Errorf("vendor bundle failed: %w", formatJSError(err, "vendor/libs.js"))
		}
	}
	...
	if _, err := vm.RunString(appBundle); err != nil {
		return nil, fmt.Errorf("app bundle failed: %w", formatJSError(err, "app bundle"))
	}
	...
	if err != nil {
		return nil, fmt.Errorf("render failed: %w", formatJSError(err, "render()"))
	}
