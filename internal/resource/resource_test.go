...
func (s *InstallableResourceSuite) SetupSuite() {
	...
	s.cmpOpts = cmp.Options{
		...
		cmpopts.SortSlices(func(a, b *resource.InternalDependency) bool {
			return s.spew.Sdump(a) < s.spew.Sdump(b)
		}),
	}
}
...
type installableResourceTestCase struct {
	name string
	// FIXME(ilya-lesikov): fix all skipped tests
	skip       bool
	inputFunc  func() *spec.ResourceSpec
	expectFunc func(resSpec *spec.ResourceSpec) *resource.InstallableResource
}
	...
	testCases := []installableResourceTestCase{
		...
		},
		{
			...
			expectFunc: func(resSpec *spec.ResourceSpec) *resource.InstallableResource {
				...
				res.Ownership = common.OwnershipEveryone
...
func (s *InstallableResourceSuite) TestNewInstallableResourceForOwnership() {
	testCases := []installableResourceTestCase{
		{
			name: `for resource with werf.io/ownership="everyone"`,
			inputFunc: func() *spec.ResourceSpec {
				resSpec := defaultResourceSpec(s.releaseNamespace)
				resSpec.SetAnnotations(lo.Assign(resSpec.Annotations, map[string]string{
					"werf.io/ownership": "everyone",
				}))
				return resSpec
			},
			expectFunc: func(resSpec *spec.ResourceSpec) *resource.InstallableResource {
...
func runDeletableResourceTest(tc deletableResourceTestCase, s *DeletableResourceSuite) func() {
	return func() {
		...
		res := resource.NewDeletableResource(resSpec.ResourceMeta, s.releaseNamespace, resource.DeletableResourceOptions{})
